...
public class RunITM {
	 ...
	 * @throws IOException
	 */
	public static double runITM(String domain) throws IOException {
		...
		Map<Integer, Map<Integer, Map<Integer, Double>>> knowledge = readKnowledge(vocab);

		ITM itm = new ITM(docs, V, knowledge);

		itm.markovChain(K, alpha, beta, eta, iterations);

		double[][] phi = itm.estimatePhi();
		...
		String filename = "file//" + domain + "_itm.txt";
		...
		double[][] theta = itm.estimateTheta();
